...
`bun run lint` runs Biome and checks `src/upstream/readme.md` for disallowed
HTML elements; both checks are local and require no network access. `bun run
check:readme` verifies that `src/upstream/readme.md` matches the upstream
...
[DEPLOYING.md](./DEPLOYING.md)). `src/upstream/` holds files synced from
other repositories rather than website-specific implementation.
